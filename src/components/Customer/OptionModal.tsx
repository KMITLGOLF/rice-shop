...
interface OptionModalProps {
  ...
  onConfirm: (item: MenuItemData, selectedOption: string) => void;
}
...
export const OptionModal: React.FC<OptionModalProps> = ({
  ...
  const handleConfirm = () => {
    if (!selectedOption) {
      alert('กรุณาเลือกเนื้อสัตว์ / ออปชันก่อนยืนยัน');
      return;
    }
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in slide-in-from-bottom-5">
        ...
        <div className="relative h-40 bg-slate-100 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-4 text-white">
            ...
            <p className="text-xs text-orange-300 font-bold mt-0.5">฿{item.price.toFixed(0)}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-5 space-y-4">
              ...
              {options.map((option) => {
                ...
                return (
                  ...
                  >
                    <span>{option}</span>
                    {isSelected && (
                      <span className="w-4 h-4 rounded-full bg-orange-500 text-white flex items-center justify-center text-[10px]">
          ...
          >
            เพิ่มลงตะกร้า • ฿{item.price.toFixed(0)}
          </button>
        </div>
      </div>
    </div>
